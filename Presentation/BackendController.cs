...
namespace Presentation
{
    public class BackendController
    {
        ...
        {
            Response<IntroSE.Kanban.Backend.ServiceLayer.Task> rs = service.AddTask(email, title, description, dueDate);
            if (rs.ErrorOccured)
                throw new Exception(rs.ErrorMessage);
        ...
        public void ChangeColumnName(string email, int ordinal, string newName)
        {
            Response rs = service.ChangeColumnName(email, ordinal, newName);
            if(rs.ErrorOccured)
                throw new Exception(rs.ErrorMessage);
        }
